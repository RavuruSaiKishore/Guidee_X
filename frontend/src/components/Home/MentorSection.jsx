        ...
        <div className="mb-8 flex flex-col gap-5 sm:mb-10 md:mb-12 md:flex-row md:items-end md:justify-between">
          <div className="max-w-2xl">
            ...
            <h2 className="mt-2 text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl md:text-4xl">
              Learn From{" "}
              <span className="text-slate-900">Industry Leaders</span>
            </h2>
        ...
        {loading ? (
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {[1, 2, 3, 4].map((item) => (
              <div
                key={item}
                className="h-[400px] animate-pulse rounded-2xl bg-slate-100 border border-slate-200"
              />
            ))}
          </div>
        ) : mentors.length > 0 ? (
          <>
            {/* DESKTOP / TABLET CARDS (Clean Minimalist White Card UI like Great Learning reference) */}
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {mentors.map((mentor) => {
                ...
                return (
                  <div
                    key={mentor._id}
                    className="group flex flex-col justify-between overflow-hidden rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl sm:block text-center"
                  >
                    <div>
                      ...
                      <div className="mt-6 space-y-1 py-4 border-t border-slate-100">
                      ...
                      {/* Transition Arrow Indicator Icon matching screenshot */}
            ...
            <div className="space-y-4 sm:hidden">
              {mentors.map((mentor) => {
                ...
                return (
                  <div
                    key={mentor._id}
                    className="overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 shadow-sm text-center"
                  >
